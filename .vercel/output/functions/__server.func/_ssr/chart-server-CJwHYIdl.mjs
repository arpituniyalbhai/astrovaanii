...
import { t as SwissEph } from "../_libs/swisseph-wasm.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/chart-server-CJwHYIdl.js
...
async function calculateChart(birth) {
	...
	const jd = swe.julday(utc.getUTCFullYear(), utc.getUTCMonth() + 1, utc.getUTCDate(), utc.getUTCHours() + utc.getUTCMinutes() / 60);
	const flags = swe.SEFLG_SWIEPH | swe.SEFLG_SIDEREAL;
	swe.set_topo(birth.longitude, birth.latitude, 0);
	const housesResult = swe.houses_ex(jd, flags, birth.latitude, birth.longitude, "W");
	const ascendant = housesResult.ascmc[0];
	Array.from(housesResult.cusps).slice(1, 13);
	const planetMap = {};
	...
	for (const p of planetDefs) {
		const lon = swe.calc_ut(jd, p.id, flags)[0];
		if (p.name === "Rahu") {
...
var getChart = createServerFn({ method: "GET" }).validator((data) => data).handler(getChart_createServerFn_handler, async ({ data }) => {
	try {
		return {
			success: true,
			chart: await calculateChart(data)
		};
	} catch (error) {
